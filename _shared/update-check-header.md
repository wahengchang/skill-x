...
執行本技能前，先從目前 `SKILL.md` 的實體路徑向上尋找 skill-x repository，並執行其 `bin/update-check`。
...
- 使用者同意：執行同一 repository 的 `bin/skill-x update --yes`，成功後繼續技能。
- 使用者拒絕：執行 `bin/snooze.sh`（預設延後 7 天），再繼續技能。
- 檢查或更新失敗：不要阻擋技能；只在失敗會影響本次任務時簡短告知。權威狀態可用 `bin/skill-x status` 查詢。
