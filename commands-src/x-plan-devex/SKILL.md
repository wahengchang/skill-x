...
choice, return a numbered decision brief to `x-plan` with a recommended answer.
The orchestrator owns the Owner Decision row. If that answer changes a
fingerprinted canonical scope/behavior input, `x-plan` applies the accepted edit
first, recomputes the fingerprint, accepts the decision at the new fingerprint,
and re-dispatches affected facets.
